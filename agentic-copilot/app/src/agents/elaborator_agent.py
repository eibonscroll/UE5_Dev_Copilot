...
class ElaboratorConfig(BaseModel):
    ...
    # strong (escalation)
    strong_model: str = os.getenv("OPENAI_STRONG_MODEL", "gpt-4.1")
    strong_temperature: float = float(os.getenv("OPENAI_TEMPERATURE", "0.2"))
    # how many docs to pull per item (plugin/module/etc.)
    per_item_k: int = int(os.getenv("ELABORATOR_PER_ITEM_K", "8"))
    # escalation gate
    min_chars: int = int(os.getenv("ELABORATOR_MIN_CHARS", "320"))
    expect_citations: bool = True  # require “(source: …)” in sections

def _llm_with(model: str, temperature: float):
    return ChatOpenAI(model=model, temperature=temperature, timeout=60)

def _needs_escalation(text: str, min_chars: int, expect_citations: bool) -> bool:
    if not text:
        return True
    if len(text.strip()) < min_chars:
        return True
    if expect_citations and "(source:" not in text:
        return True
    return False
...
class ElaboratorAgent:
    ...
    Strategy:
      ...
      4) If a section looks weak, retry once with a stronger model.
    """
    def __init__(self, retriever, cfg: ElaboratorConfig | None = None):
        self.retriever = retriever
        self.cfg = cfg or ElaboratorConfig()
        # LLMs
        self.fast_llm = _llm_with(self.cfg.model, self.cfg.temperature)
        self.strong_llm = _llm_with(self.cfg.strong_model, self.cfg.strong_temperature)
        ...
        # Build chains per-LLM so we can escalate
        self.fast_chain = self.base_prompt | self.fast_llm | StrOutputParser()
        self.strong_chain = self.base_prompt | self.strong_llm | StrOutputParser()
    ...
    def _retrieve_for_item(self, item_name: str, per_item_k: int) -> List[Dict[str, Any]]:
        ...
        for it in self._focused_queries(item_name):
            # Prefer the new invoke() API; fall back if needed
            try:
                docs = self.retriever.invoke(it)  # new API
            except Exception:
        ...
        return deduped[: max(12, per_item_k)]

    def _summarize_once(self, item_label: str, name: str, ctx_snippets: List[Dict[str, Any]], use_strong: bool = False) -> str:
        numbered = []
        for i, r in enumerate(ctx_snippets, 1):
        ...
        chain = self.strong_chain if use_strong else self.fast_chain
        out = (chain.invoke({"user_input": user_input}) or "").strip()
        return out
    ...
    def elaborate(self, base_md: str, item_label: str, items: List[str]) -> str:
        ...
        for name in items:
            ...
            summary = self._summarize_once(item_label, name, ctx, use_strong=False)

            # Escalate if needed
            if _needs_escalation(summary, self.cfg.min_chars, self.cfg.expect_citations):
                summary2 = self._summarize_once(item_label, name, ctx, use_strong=True)
                if summary2:
                    summary = summary2
